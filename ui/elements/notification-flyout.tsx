...
const solutions = [
  { name: 'Analytics', description: 'Get a better understanding of your traffic', href: '#', icon: MegaphoneIcon },
  { name: 'Analytics 2', description: 'Get a better understanding of your traffic', href: '#', icon: MegaphoneIcon }
]
const callsToAction = [
  { name: 'Notifications', href: '/dashboard/notifications', icon: EyeIcon }
]
...
export default async function NotificationFlyout() {
  ...
  return (
    <div>
              ...
              {solutions.map((item) => (
                <div key={item.name} className="w-fit group relative flex gap-x-4 rounded-lg p-1 hover:bg-gray-50">
                  <div className="flex size-9 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-white">
                    <item.icon aria-hidden="true" className="size-6 text-gray-600 group-hover:text-brand-600" />
                  </div>
                  <div>
      ...
      <Link href={"/dashboard/notifications"} className="md:hidden text-brand-300 hover:cursor-pointer">
          <BellIcon aria-hidden="true" className="size-7" />
      </Link>
